window.switchStatsTab = function(tab) {
    ...
    if (tab === 'stats') {
        ...
        if (trendsPanel) trendsPanel.style.display = 'block';
        renderStatsTrends(currentTrendRange || 7);
    } else {
                ...
                <div style="background:var(--secondary-bg);border-radius:20px;padding:24px;width:88%;max-width:380px;box-shadow:0 20px 60px rgba(0,0,0,0.4);animation:modalContentSlideIn 0.3s ease forwards;">
                    ...
                    <div style="display:flex;flex-direction:column;gap:9px;margin-bottom:20px;">
                        <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:10px 12px;border:1px solid var(--border-color);border-radius:12px;background:var(--primary-bg);font-size:13px;color:var(--text-primary);">
                            ...
                            <span>聊天记录 <span style="font-size:11px;color:var(--text-secondary);">(${messages.letypeof messages !== 'undefined' ? messages.length : 0} 条)</span></span>
                        </label>
                        <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:10px 12px;border:1px solid var(--border-color);border-radius:12px;background:var(--primary-bg);font-size:13px;color:var(--text-primary);">
...
function renderStatsTrends(days) {
    ...
    const hourlyCount = new Array(24).fill(0).map(() => ({ user:0, partner:0 }));
    ...
    messages.forEach(msg => {
        ...
        if (!dailyMap.has(dateKey)) dailyMap.set(dateKey, { user:0, partner:0 });
        const entry = dailyMap.get(dateKey);
        if (msg.sender === 'user') entry.user++;
        else entry.partner++;
        const hour = ts.getHours();
        if (msg.sender === 'user') hourlyCount[hour].user++;
        else hourlyCount[hour].partner++;
    });
    
    const sortedDates = Array.from(dailyMap.keys()).sort();
    const labels = sortedDates.map(d => d.slice(5));
    const userData = sortedDates.map(d => dailyMap.get(d).user);
    const partnerData = sortedDates.map(d => dailyMap.get(d).partner);
    
    // 获取实际颜色值
    const root = getComputedStyle(document.documentElement);
    const accentColor = root.getPropertyValue('--accent-color').trim();
    const accentRgb = root.getPropertyValue('--accent-color-rgb').trim();
    
    const ctxTrend = document.getElementById('trends-chart').getContext('2d');
    ...
    trendsChart = new Chart(ctxTrend, {
        ...
        data: {
            labels: labels,
            datasets: [
                { label: '我', data: userData, borderColor: accentColor, backgroundColor: `rgba(${accentRgb},0.1)`, tension: 0.3, fill: true },
                { label: '对方', data: partnerData, borderColor: '#ff6b6b', backgroundColor: 'rgba(255,107,107,0.1)', tension: 0.3, fill: true }
            ]
        },
    ...
    hourlyChart = new Chart(ctxHour, {
        ...
        data: {
            ...
            datasets: [
                { label: '我', data: hourlyCount.map(h => h.user), backgroundColor: `rgba(${accentRgb},0.6)` },
                { label: '对方', data: hourlyCount.map(h => h.partner), backgroundColor: 'rgba(255,107,107,0.6)' }
            ]
        },
        options: { responsive: true, maintainAspectRatio: true, plugins: { legend: { position: 'top' } }, scales: { x: { ticks: { stepSize: 2, maxRotation: 45 } } } }
    });
}
...
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.range-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            renderStatsTrends(days);
            document.querySelectorAll('.range-btn').forEach(b => b.classList.remove('modal-btn-primary'));
            this.classList.add('modal-btn-primary');
        });
    });
    // 默认高亮近7天按钮
    const defaultRangeBtn = document.querySelector('.range-btn[data-range="7"]');
    if (defaultRangeBtn) defaultRangeBtn.classList.add('modal-btn-primary');
});
