(function () {
    ...
    const KEY_RINGTONE_PRESET = 'callRingtonePreset';
    const KEY_RINGTONE_CUSTOM = 'callRingtoneCustom';
    ...
    const S = {
        ...
        isPartnerCall:   false,
        ringtonePreset: localStorage.getItem(KEY_RINGTONE_PRESET) || 'default',
        ringtoneCustom: localStorage.getItem(KEY_RINGTONE_CUSTOM) || null,
        ringtoneAudio: null,   // 新增：保存正在播放的铃声音频对象
    };

// 在 const S = { ... }; 之后、init 之前添加
...
function playRingtone() {
    if (S.ringtonePreset === 'mute') return;
    stopRingtone();
    let url = null;
    if (S.ringtoneCustom && S.ringtoneCustom.trim()) {
        url = S.ringtoneCustom;
    } else {
        const presetMap = {
            default: 'https://www.soundjay.com/phone/phone-ringtone-1.mp3',
            classic: 'https://www.soundjay.com/phone/classic-phone-ringtone-1.mp3',
            digital: 'https://www.soundjay.com/phone/digital-phone-ringtone.mp3'
        };
        url = presetMap[S.ringtonePreset] || presetMap.default;
    }
    if (!url) return;
    ...
    _ringtoneAudio.play().catch(e => {
        ...
        if (typeof showNotification === 'function') {
            showNotification('铃声加载失败，请检查自定义链接是否有效', 'error', 3000);
        }
    });
}
  ...
  <button class="call-mini-hangup" id="call-mini-hangup">${SVG_HU}</button>
    ...
    function scheduleRandomCall() {
        ...
        S.randomCallTimer = setTimeout(() => {
            if (S.enabled && !S.active && Math.random() < 0.20) showIncomingCall();
            scheduleRandomCall();
        }, ms);
    }
